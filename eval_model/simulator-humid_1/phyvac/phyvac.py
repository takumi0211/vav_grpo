...
class Damper:
    ...
    def _coef_at_position(self):
        n = len(self.coef)
        if n == 0:
            return 0.0
        if self.damp >= self.coef[0][0]:
            return float(self.coef[0][1])
        if self.damp <= self.coef[n - 1][0]:
            return float(self.coef[n - 1][1])
        for i in range(1, n):  # 線形補間の上限・下限となる曲線を探す
            ...
            if coef_b[0] <= self.damp < coef_a[0]:
                span = coef_a[0] - coef_b[0]
                if span <= 0:
                    return float(coef_a[1])
                ratio = (self.damp - coef_b[0]) / span
                return float(coef_b[1] + (coef_a[1] - coef_b[1]) * ratio)
        return float(self.coef[n - 1][1])
    ...
    def f2p_co(self):
        coef = self._coef_at_position()
        return np.array([0.0, 0.0, -coef])
...
class PID:
    ...
    def __init__(self, mode=1, a_max=1, a_min=0, kp=0.8, ti=10, sig=0, t_reset=30, kg=1, t_step=1,a=0.0, max_step: float = 0.05):
        # mode          :運転時1, 非運転時0
        ...
        self.a = a
        # 1ステップあたりの制御量変化上限（絶対値）。従来は±0.05固定。
        # 外部から可変にすることで応答性・安定性の調整を容易にする。
        try:
    ...
    def control(self, sp, mv):
        if self.mode == 1:
            ...
            if self.t_step_cnt % self.t_step == 0:
                ...
                # 1ステップの変化上限（従来±0.05）
                max_step = self.max_step if hasattr(self, "max_step") else 0.05
                if ctrl > max_step:
                    ctrl = max_step
                elif ctrl < -max_step:
                    ctrl = -max_step
